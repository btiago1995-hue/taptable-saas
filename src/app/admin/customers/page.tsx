...
import { Search, Filter, Mail, MessageSquare, ArrowUpDown, ChevronDown } from "lucide-react";
...
export default function AdminCustomers() {
    ...
    const filteredCustomers = mockCustomers.filter(c => {
        if (activeSegment === "vip") return c.status === "Vip";
        if (activeSegment === "risk") return c.status === "Em Risco";
        return true;
    });
        ...
        <div className="max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2">
                    <button className="bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl font-semibold flex items-center gap-2 hover:bg-slate-50 transition-colors">
                        <Filter className="w-4 h-4" /> Filtros
                    </button>
                    <button className="bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition-colors">
                        Exportar CSV
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                    { label: "Total na Base", value: "0" },
                    { label: "Clientes VIP (+10 visitas)", value: "0" },
                    { label: "Ticket Médio Global", value: formatCurrency(0) },
                    { label: "Taxa de Retenção", value: "0%" },
                ].map((kpi, i) => (
                    ...
                    <div className="flex gap-2 bg-slate-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto scrollbar-hide">
                        {[
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors ${activeSegment === tab.id
                ...
                <div className="overflow-x-auto">
                                ...
                                <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors tooltip" title="Enviar SMS">
                                                <MessageSquare className="w-4 h-4" />
                                            </button>
                                            <button className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors tooltip" title="Enviar E-mail">
                    ...
                    {filteredCustomers.length === 0 && (
                        <div className="p-12 text-center text-slate-500">
                            Nenhum cliente encontrado nessa visão.
                        </div>
                    )}
                </div>
